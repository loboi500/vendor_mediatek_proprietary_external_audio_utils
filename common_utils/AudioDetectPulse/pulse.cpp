...
void detectPulse(const int TagNum, const int pulseLevel, const int dump, void *ptr,
    ...
    if (dump) {
        // dump pcm
        char  fileName[80] = {0};
        if (sprintf(fileName, "%s.%s.pcm", "/sdcard/mtklog/audio_dump/detectPulse_16bit_Tag", Tag2String(TagNum)) < 0) {
            ALOGE("%s, %s, Create dump file name error!!", __FUNCTION__, Tag2String(TagNum));
        } else {
