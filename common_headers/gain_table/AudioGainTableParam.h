...
#define CRS_VOL_AUDIOTYPE_NAME "CRSVol"
...
#define GAIN_MAX_STREAM_TYPE (AUDIO_STREAM_ASSISTANT)
...
struct GainTableMicUnit {
    char gain[TOTAL_MIC_IN_CHANNEL];
};
...
struct GainTableForNonScene{
    ...
    GainTableCRSToneUnit crsGain[NUM_GAIN_DEVICE][GAIN_VOL_INDEX_SIZE];
};
...
struct GainTableSpec {
    ...
    std::string audioBufMixerName;
    std::string spkMixerName;
    ...
    int ulPgaGainMapMax;
    int ulHwPgaIdxMax;
    std::string ulPgaMixerName[TOTAL_MIC_IN_CHANNEL];
